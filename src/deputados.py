...
def bronze(data_inicio: str, data_fim: str):
    data = baixar_dados_paginados(
        endpoint="deputados", data_inicio=data_inicio, data_fim=data_fim
    )
    return data
...
def gold(data_inicio: str, data_fim: str):
    ...
    salvar_dados_postgres(df, tabela="deputados", chave_primaria="id")
    logger.success("Pipeline Deputados concluído com sucesso!")
